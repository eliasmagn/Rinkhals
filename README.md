...
- Experimental MMU Ace integration to prepare Anycubic's multi-material unit workflows, now mirroring live spool status, supporting multiple ACE hubs, exposing dryer controls, and syncing endless-spool groups
...
| Kobra S1 (+ combo) | `2.5.6.0` `2.5.6.4` |
...
- Supported slicers stream tool-map updates through `MMU_SLICER_TOOL_MAP`; the bridge now parses both JSON and comma-separated payloads so Moonraker reflects the latest tool metadata instantly.
